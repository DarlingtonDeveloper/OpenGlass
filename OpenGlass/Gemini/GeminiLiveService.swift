...
class GeminiLiveService: ObservableObject {
    ...
    // Dynamic configuration for mode switching
    ...
    func connect() async -> Bool {
        guard let url = OpenGlassConfig.websocketURL() else {
    ...
    func sendVideoFrame(image: UIImage) {
        guard connectionState == .ready else { return }
        sendQueue.async { [weak self] in
            guard let jpegData = image.jpegData(compressionQuality: OpenGlassConfig.videoJPEGQuality) else { return }
